...
var _ = Describe("[IPAM]", func() {
	...
	Describe("[IPAM]", func() {
		...
		It("normal subnet", func() {
			...
			err = im.GetStaticAddress("pod1.ns", "10.16.0.2", subnetName)
			Expect(err).ShouldNot(HaveOccurred())

			ip, err := im.GetRandomAddress("pod1.ns", subnetName)
			Expect(err).ShouldNot(HaveOccurred())
			Expect(ip).To(Equal("10.16.0.2"))

			ip, err = im.GetRandomAddress("pod2.ns", subnetName)
			Expect(err).ShouldNot(HaveOccurred())
			Expect(ip).To(Equal("10.16.0.3"))

			err = im.GetStaticAddress("pod3.ns", "10.16.0.2", subnetName)
			Expect(err).Should(MatchError(ipam.ConflictError))

			im.ReleaseAddressByPod("pod1.ns", subnetName)
			err = im.GetStaticAddress("pod3.ns", "10.16.0.2", subnetName)
			Expect(err).ShouldNot(HaveOccurred())

			_, err = im.GetRandomAddress("pod4.ns", "invalid_subnet")
			Expect(err).Should(MatchError(ipam.NoAvailableError))

			err = im.AddOrUpdateSubnet(subnetName, cidrStr, []string{})
			Expect(err).ShouldNot(HaveOccurred())

			ip, err = im.GetRandomAddress("pod5.ns", subnetName)
			Expect(err).ShouldNot(HaveOccurred())
	...
	Describe("[Subnet]", func() {
		...
		It("static allocation", func() {
			subnet, err := ipam.NewSubnet(subnetName, cidrStr, excludeIps)
			Expect(err).ShouldNot(HaveOccurred())
			Expect(subnet.GetStaticAddress("pod1.ns", "10.16.0.2", false)).ShouldNot(HaveOccurred())
			Expect(subnet.GetStaticAddress("pod2.ns", "10.16.0.3", false)).ShouldNot(HaveOccurred())
			Expect(subnet.GetStaticAddress("pod3.ns", "10.16.0.20", false)).ShouldNot(HaveOccurred())
			Expect(subnet.FreeIPList).To(Equal(
				ipam.IPRangeList{
					&ipam.IPRange{Start: "10.16.0.5", End: "10.16.0.9"},
					&ipam.IPRange{Start: "10.16.0.24", End: "10.16.255.254"},
			...
			Expect(subnet.GetStaticAddress("pod4.ns", "10.16.0.3", false)).Should(MatchError(ipam.ConflictError))
			Expect(subnet.GetStaticAddress("pod5.ns", "19.16.0.3", false)).Should(MatchError(ipam.OutOfRangeError))
		...
		It("random allocation", func() {
			...
			ip1, err := subnet.GetRandomAddress("pod1.ns")
			Expect(err).ShouldNot(HaveOccurred())
			...
			ip2, err := subnet.GetRandomAddress("pod2.ns")
			Expect(err).ShouldNot(HaveOccurred())
			Expect(ip2).To(Equal(ipam.IP("10.16.0.2")))

			_, err = subnet.GetRandomAddress("pod3.ns")
			Expect(err).Should(MatchError(ipam.NoAvailableError))
